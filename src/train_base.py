...
hyperpara = ' c_1stDiv {}\n centroid_d {}\n sr {}\n bitrate {}\n lr {}\n weight1 {}\n mel_weight {}\n window_in_data {}'.format(filters, d, sr, br, lr, weight1, mel_weight, window_in_data)
print(hyperpara)
...
if not isinstance(saved_model, type(None)):
    ...
    model = Model(block = Bottleneck, scale = 10, filters = filters, d = d,  num_m = m, sr = sr).cuda()
...
while 1: 
    ...
    if t == 10 and model.stage == 0:
        if not debugging:
            ...
            with open(result_path, 'a') as the_file:
                the_file.write('saved at ../models/{}_stage0.model'.format(model_name))
        model.stage = 1
        model.max_score = 0
        itermax = 0
    if model.scale < 500:
    ...
    for wave_s, wave_x, source, mixture in train_loader:
        ...
        x_h, prob = model(mixture, soft = True)
        
        rx = rebuild_f(x_h).unsqueeze(dim = 0)

        train_mel_mx = melMSELoss_short(x_h.cpu().data, mixture.cpu().data)
        ...
        loss = criterion(x_h, mixture)\
#         + mel_weight * train_mel_mx
        
        if model.etp == 1:
            control = 1
            loss += weight1 * ((target - entp)**2).cuda()
...
#             break
    ...
    h_x_score, h_rx_score, max_single_rx, arg = test_base(False, test_loader, model, window_in_data, debugging)
