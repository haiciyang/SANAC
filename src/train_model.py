...
from Prop import Prop_Model as Model
from utils import *
import glob

debugging = True # Won't write out results in file or save models if debugging is True
...
model_name = '0927_170108_32_d6_5db'
...
hyperpara = ' c_1stDiv {}\n c_2ndDiv {}\n centroid_d {}\n sr {}\n bitrate {}\n lr {}\n weight1 {}\n weight2 {}\n ratio {}\n'.format(filters, f2, d, sr, br, lr, weight1, weight2, ratio)
...
if model_path in glob.glob('../models/*.model'):
    ...
    print('loaded trained model: '+ model_name+'\n')
else:
...
# model = torch.load('0513_210109ratio_sr32_d6_5db.model')
...
#                 loss += 1/2 * ((target - entp_cl - entp_ns) ** 2).cpu()
    ...
    Epoch_info = 'epoch_{}| Time:{:.0f} | Control:{:.0f} | Stage: {} | Itermax: {} | etp:{} '.format(t, end-start, control, model.stage, itermax, etp)
    print(Epoch_info)
    if not debugging:
...
#     if model.stage == 1 and itermax >= 2:
    if model.stage == 1 and itermax >= 5 and etp == 1 :
        model.stage = 2  
        itermax = 0
#         max_sdr = (0,0,0,0)
        print('Enter stage 2')

    if etp == 1 and itermax > 20:
        print('over')
        break
