...
#         if self.task == 'train':
    ...
    def array_append(self, c, n):
        ...
        x = x[:len(c)]
#         sections_n = np.random.randint(len(self.noise), size = len(c_l))
#         permu = np.random.randint(len(c_l), size = len(c_l))       
#         n = self.noise[sections_n]
#         if self.task == 'train':
#             c_l = c_l[permu]
#             x_l = x_l[permu]
#         elif self.task == 'test':
#             c_l = c_l[len(c_l)//4 : len(c_l)//4 + len(c_l)//2]
#         sections = np.random.randint(len(c), size = mix_num * size)
