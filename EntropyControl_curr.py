...
#         self.mask = torch.rand((d, 512), device = 'cuda:0', requires_grad = True)
        ...
        if block:
            ...
            enc_layers = []
            enc_layers.append(BasicBlock(1, self.filters))
            ...
            for i in range(layers-1):
                enc_layers.append(BasicBlock(self.d_s, self.d_s))
...
#                 block(self.d_n, self.d_n)
    ...
    def forward_half_std(self, x, soft=True): # Coding first
...
#         print(x.dtype)
        ...
        if self.stage >= 1:
            ...
            if self.stage == 2:
                ...
                n_hat = self.dec_2n(code_n) # -- shape (bs, d, 512)
                ...
                n_hat = torch.tanh(self.fc_2n(n_hat))
                
                return s_hat, n_hat , arg_idx_s, arg_idx_n                

        s_hat = self.dec_1s(code_s) # -- shape (bs, d, 512)
        n_hat = self.dec_1n(code_n) # -- shape (bs, d, 512)

        s_hat = s_hat.view(-1, s_hat.shape[1] * s_hat.shape[-1])
        n_hat = n_hat.view(-1, n_hat.shape[1] * n_hat.shape[-1])
        s_hat = torch.tanh(self.fc_1s(s_hat))
        n_hat = torch.tanh(self.fc_1n(n_hat))
                
        return s_hat, n_hat , arg_idx_s, arg_idx_n
    
    def forward_half_std_share(self, x, soft=True): # Coding first
...
#         print(x.dtype)
        ...
        s_hat = self.dec_1s(code_s) # -- shape (bs, d, 512)
        n_hat = self.dec_1s(code_n) # -- shape (bs, d, 512)

        s_hat = s_hat.view(-1, s_hat.shape[1] * s_hat.shape[-1])
        n_hat = n_hat.view(-1, n_hat.shape[1] * n_hat.shape[-1])
        s_hat = torch.tanh(self.fc_1s(s_hat))
        n_hat = torch.tanh(self.fc_1s(n_hat))
                
        return s_hat, n_hat , arg_idx_s, arg_idx_n
    
    
    
    def baseline(self, x, soft=True): # Coding first
...
#                 return s_hat, n_hat , arg_idx_s, arg_idx_n
        ...
        if self.sr:
            s_hat = self.dec_sr_in(code) # -- shape (bs, d//2, 256)
